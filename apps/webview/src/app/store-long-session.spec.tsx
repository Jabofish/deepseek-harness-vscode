...
describe('Desensitized long-session replay', () => {
  ...
  // The replay walks ~200 fixture frames with a settle between each one to model
  // streamed delivery; under a fully parallel suite that exceeds the default 5s
  // test timeout even though the assertions are deterministic.
  const REPLAY_TIMEOUT_MS = 30_000

  it(
    'presents six turns, eight tool types, todos, deliverables, and the final answer',
    async () => {
  ...
  it(
    'keeps every answer in place across rerenders and a below-cursor redelivery',
    async () => {
      const client = new LongSessionClient()
      const store = createAppStore(client as unknown as ProtocolClient)
      await store.openSession(LONG_SESSION_ID)
