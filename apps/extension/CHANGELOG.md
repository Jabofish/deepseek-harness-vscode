...
- 会话切换器新增「已归档」分区：按需向宿主读取归档列表，逐行恢复，永久删除必须在确认对话框中确认；被宿主拒绝的读取、恢复和删除都按宿主原文说明原因，不再静默失败。
- 插件配置不再内置插件清单：命名空间、字段、可选值和凭据状态全部来自宿主 `settings.describe`，布尔与枚举字段改用选择控件；凭据只在宿主命名空间自己声明的引用下写入，不再猜测环境变量名。
- 运行状态面板显示宿主的特性适配概况：每个能力 ID 的「已验证 / 兼容回退 / 不可用」与整体来源（精确适配或兼容回退）都在面板内陈述。
- 任务中心只投影契约能证明的会话取消，不再显示无法实现的进程停止动作；预设位置入口只在宿主陈述该项能力时区分「原生打开」与「显示路径」。
- 移除了一批无法到达的 Webview 路由与组件（工作区创建、设置整份替换、目标创建、技能刷新/执行、预设选择、右栏引导请求等），协议声明与宿主分发保持一致。
- Add an "Archived" section to the session switcher: the archived list is read from the host on demand, rows can be restored one by one, and a permanent delete has to be confirmed in a dialog; a refused read, restore, or delete states the host's own reason instead of failing silently.
- The plugin configuration ships no plugin list: namespaces, fields, choice values, and credential state all come from the host's `settings.describe`, boolean and enum fields render as choice controls, and a secret is only written under the reference the host namespace itself states.
- The runtime status panel states the host's feature readiness: every capability ID's verified/fallback/unavailable verdict and the overall pinned-or-fallback source are shown in the panel.
- The task centre projects only the session cancel the contract can prove (no unimplemented process-stop action), and the preset location action distinguishes a native opener from a revealed path only when the host states that capability.
- Removed a set of unreachable Webview routes and components (workspace create, whole-settings replace, goal create, skill refresh/execute, preset select, right-side guide request), keeping the declared protocol and the host dispatch in step.
...
- 修复 Webview 主题覆盖的缺口：设置 → 插件 → 插件配置里的布尔/枚举下拉此前是没有样式的原生控件，现在与同列输入框同一高度、边框、留白和焦点环，模板对话框的变量框不再带平台原生边框，信息态状态药丸、检查点冲突提示、设置「打开设置文档」一行以及工具卡的 diff 断层/文件数、搜索命中数与合计、读取范围也都补上了样式。
- Fix gaps in the Webview theme coverage: the boolean/enum dropdown in Settings → Plugins → plugin configuration was an unstyled native control and now matches the inputs beside it in height, border, padding, and focus ring, the template dialog's variables fieldset no longer carries the platform's native border, and the informational status pill, the checkpoint conflict notice, the settings document row, and the tool card's diff gap/file count, search hit count and total, and read range are styled too.
- 重写 DSH 连接进度页，改为启动骨架屏：状态灯、标题与设置入口同处一行，下面是确定型进度条（带百分比读数）和五行会话列表占位骨架（微光交错扫过），不再逐步叙述五个阶段，阶段标签只留给读屏；连接失败时骨架整块收起，原地换成危险色原因框与重试 / 连接设置 / 诊断动作。
- Rewrite the DSH connection progress page as a startup skeleton screen: a status light, the title and a settings entry share one header row above a determinate progress bar with a percentage readout and five placeholder session rows that shimmer in sequence, replacing the step-by-step stage narration (the stage labels stay for screen readers only); a failed connection collapses the skeleton and replaces it in place with the danger-toned reason and the retry / connection settings / diagnostics actions.
- 修复连接阶段回退：自动连接不再对每个被发现的实例提前宣告「正在连接」，只有本扩展已启动并持有进程、真正要去附加时才进入该阶段；探测到一个不健康的实例再回落到定位运行时/启动，因此不再出现进度先跳到「正在连接」又退回「定位运行时」的倒退（新测试固定 discovering → locating-runtime → starting → connecting → connected 的单调顺序）。
- Fix a backwards stage jump during connection: automatic connect no longer announces "connecting" while it is still probing discovered instances — that phase now starts only once this extension owns a process to attach to, so falling back from an unhealthy discovered instance to locating the runtime and starting it no longer rolls the progress indicator from connecting back to locating-runtime (a new test pins the strictly advancing discovering → locating-runtime → starting → connecting → connected order).
