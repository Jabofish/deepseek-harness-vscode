...
flowchart TB
  ...
  subgraph Adapter["Versioned DSH Boundary"]
    ...
    ALPHA["alpha.1 → alpha.2 → alpha.3 → alpha.4 → alpha.5 → alpha13 → alpha132 → alpha151 → alpha152 → rc151\nConnection/Gateway family + versioned Session seams"]
...
| `packages/dsh-adapter`      | `0.0.1-rc.1/.2/.5`、`0.1.0-rc.2/.3`、rc.6–0.1.2-rc.1、alpha.1–alpha.5、0.1.3-alpha.1/.2 与 0.1.5-alpha.1/.2/rc.1 的 RPC/Event 映射、仓储、流恢复 | Domain、Application ports、固定上游包   | VS Code、React                |
