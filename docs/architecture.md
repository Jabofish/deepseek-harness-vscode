...
flowchart TB
  ...
  subgraph Adapter["Versioned DSH Boundary"]
    ...
    ALPHA["Alpha.1/alpha.2 Connection/Gateway Adapters"]
...
| `packages/dsh-adapter`      | rc.6–0.1.1-rc.2 与 alpha.1/alpha.2 的 RPC/Event 映射、仓储、流恢复 | Domain、Application ports、固定上游包   | VS Code、React                |
...
packages/
  ...
  dsh-adapter/
    ...
    src/versions/alpha/   # alpha.1/alpha.2 共用 /api + remote.mux 传输
