...
flowchart TB
  ...
  subgraph Adapter["Versioned DSH Boundary"]
    ...
    ALPHA["Unpublished alpha.1 Connection/Gateway Adapter"]
...
| `packages/dsh-adapter`      | rc.6–0.1.1-rc.2 与未发布 alpha.1 的 RPC/Event 映射、仓储、流恢复 | Domain、Application ports、固定上游包   | VS Code、React                |
...
packages/
  ...
  dsh-adapter/
    ...
    src/versions/alpha/   # 0.1.2-alpha.1 /api + remote.mux 预适配
