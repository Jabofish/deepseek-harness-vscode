...
- DSH：已发布 `0.0.1-rc.1`/`.2`/`.5`、`0.1.0-rc.2`/`.3`、`0.1.0-rc.6` 至 `0.1.2-rc.1`、`0.1.2-alpha.2` 至 `0.1.2-alpha.5`、`0.1.3-alpha.2`、`0.1.5-alpha.1`/`.2`/`rc.1`/`rc.2`，以及 alpha 通道 `0.1.6-alpha.1`，并保留所有已知 tag 的独立版本入口；`0.0.1-rc.1/.2` 使用旧 Host API，`0.1.0-rc.2/.3` 与 rc.6 Host wire 等价；`0.1.3-alpha.1/.2` 使用 Session v2，`0.1.5-alpha.1/.2/rc.1/rc.2/0.1.6-alpha.1` 使用严格 Session v3，均只对精确版本启用。`.2` 额外透传严格必填的 subagent `delivery`，`.2/rc.1/rc.2` 贯通 `deliverables/presented` 与 `subagent/catalog` 投影，`alpha161` 对 `image/offload` 仅做脱敏 opaque 保留，rc.2 反馈提交支持上游分类与统一对话框语义；任何非空未知版本标签会按可安全复用已验证 wire 的 Adapter 进行只读兼容探测，未知运行时回退到 alpha5 v0，成功后保留真实版本并显示警告；真实联调前用 `dsh --version` 确认。安装器默认通过 npm `next` 使用 `0.1.5-rc.2`（npm `latest` 仍为 `rc.1`）。

版本 Adapter 结构：公共 identity/probe 形状位于 `packages/dsh-adapter/src/adapter-base.ts`；实现按真实协议边界维护 legacy rc、alpha family v0、alpha13/alpha132 Session v2 和 alpha151/alpha152/rc151/rc152/alpha161 Session v3 明确入口。legacy rc 内部再区分 `0.0.1-rc.1`、`.2` 的旧 `command.*`/事件族与 `0.0.1-rc.5`、`0.1.0-rc.2/.3` 的 rc.6 Host API 族；`0.1.2-rc.1` 虽是 rc 发布号，浏览器 wire 仍沿 alpha.5，因此由 `versions/rc13` 复用 v0；`0.1.3-alpha.1` 使用 `versions/alpha13`，`.2` 使用 `versions/alpha132` 并仅在该版本启用 subagent `delivery`，`0.1.5-alpha.1/.2/rc.1/rc.2` 使用 `versions/alpha151`/`alpha152`/`rc151`/`rc152` 并仅在对应版本启用 v3 及已核对的交付/目录事件，`0.1.6-alpha.1` 使用 `versions/alpha161` 并仅在精确入口启用其新增 projection 事件的安全 opaque 边界。新增版本必须先完成上游差异审计和脱敏契约 fixture，再决定是否需要新的 mapper；不能跨 wire family 猜测。
...
3. 托管启动使用版本化参数数组：`0.0.1-rc.1/.2/.5`、`0.1.0-rc.2/.3`、`0.1.0-rc.6/.7` 为 `--profile web --host 127.0.0.1 --port <n>`；rc.8、`0.1.1-rc.1/.2`、`0.1.2-rc.1`、alpha.1–alpha.5、`0.1.3-alpha.1/.2` 和 `0.1.5-alpha.1/.2/rc.1/rc.2` 在同一组参数中追加已由上游声明的 `--no-open`；未知版本不猜测该可选参数。
4. 扩展关闭后只结束本次扩展创建的进程。
