...
- 协议只传可序列化 DTO，不传 Error、Map、Set、AbortSignal、VS Code 对象或上游 rc.6–0.1.1-rc.2/0.1.2-alpha.1 类。
...
未发布 `0.1.2-alpha.1` 由 Extension Host 内的 `versions/alpha` 适配：新 `/api/<namespace>/<method>` Connection RPC、Cookie 握手和 `/api/remote.mux` 流均在 Host 处理，再投影为同一组 Domain/Application DTO；Webview 不接收 launch token、Cookie、endpoint 或原始上游错误。该适配不改变 Webview 协议版本，也不把未发布 alpha 加入安装默认值。
