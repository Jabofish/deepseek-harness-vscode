...
| 层          | 目标                                                                                                                                                                                                                 | 禁止替代                     |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| Contract    | `0.0.1-rc.1/.2/.5`、`0.1.0-rc.2/.3`、rc.6–0.1.2-rc.1、alpha.1–alpha.5、0.1.3-alpha.1/.2、0.1.5-alpha.1/.2/rc.1/rc.2/rc.3、0.1.6-alpha.1/.2 与 0.1.7-alpha.1/.2/rc.1 RPC/Event/Tool/Session-Control/Job 形状与 mapper | 不能只 mock Domain           |
...
- 未发布版本（如 alpha.1）的 fixture 必须同时标注源码 tag/commit，并在能力矩阵中注明尚无真实运行包证据；已发布预发行版本（如 `0.1.2-alpha.2`、`0.1.2-alpha.3`、`0.1.2-alpha.4`、`0.1.2-alpha.5`、`0.1.3-alpha.2`、`0.1.5-alpha.1/.2/rc.1/rc.2/rc.3`、`0.1.7-alpha.2`、`0.1.7-rc.1`）也要记录 npm/tag/commit 与 live smoke 边界。
...
- `0.1.7-alpha.2` fixture 必须固定到 `dsh-v0.1.7-alpha.2`，验证继承的 Session V4/Workspace/Job wire、由 `projectContent()` 插入的标准 ContentBlock、精确身份拒绝相邻版本、取消与 mux 释放；与共享分页合同测试一起验证 `session/follow` 和 `session/page` 都携带 `maxMessages:500`、`turnWindow`，且不让该字段进入不支持的版本。
- `0.1.7-rc.1` fixture 必须固定到 `dsh-v0.1.7-rc.1`，确认已发布 web profile 实际启用 Job Controller，验证 exact-only 探测、Session V4/pinned Workspace 基线、turn-window 的 Session follow/page 参数、错误/取消及 mux 释放；不得根据旧 `0.1.5-rc.1` Jobs 边界推断这个独立版本不支持 Job Controller。
- `0.1.5-rc.3` fixture 必须固定到 `dsh-v0.1.5-rc.3`，保留独立精确身份，并对照 `dsh-v0.1.5-rc.2` 检查适配实际消费的 `packages/api/session-controller/src/{index.ts,types.ts,history.ts,remote-events.ts,client/transport.ts,client/contract/events.ts}`、`packages/api/remotes/src/{client/index.ts,types.ts}`、`packages/api/gateway/src/client/{remote-stream.ts,stream-client.ts,remote-events.ts}`、`packages/client/connection/src/client/index.ts` 和 `packages/core/tools/src`；这些运行时源码在两个 tag 间一致，rc.3 的差异为包元数据、lockfile 与依赖策略工具。固定 SHA `47f943...` 的 `rpc-map.ts`/`events.ts` 属于旧 Host API 历史基线，在 rc.2/rc.3 tag 中不存在，不能用作这两个版本的比较证据；也不能因为版本递增而把未来 rc 版本纳入兼容回退。
