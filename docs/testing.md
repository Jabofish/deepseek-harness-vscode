...
| 层          | 目标                                                                              | 禁止替代                     |
| ----------- | --------------------------------------------------------------------------------- | ---------------------------- |
| Contract    | rc.6–0.1.2-rc.1、alpha.1–alpha.5 与 0.1.3-alpha.1/.2 RPC/Event/Tool 形状与 mapper | 不能只 mock Domain           |
...
- 未发布版本（如 alpha.1）的 fixture 必须同时标注源码 tag/commit，并在能力矩阵中注明尚无真实运行包证据；已发布预发行版本（如 `0.1.2-alpha.2`、`0.1.2-alpha.3`、`0.1.2-alpha.4`、`0.1.2-alpha.5`、`0.1.3-alpha.2`）也要记录 npm/tag/commit 与 live smoke 边界。
