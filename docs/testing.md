...
| 层          | 目标                                                                                                                                                                              | 禁止替代                     |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| Contract    | `0.0.1-rc.1/.2/.5`、`0.1.0-rc.2/.3`、rc.6–0.1.2-rc.1、alpha.1–alpha.5、0.1.3-alpha.1/.2、0.1.5-alpha.1/.2/rc.1/rc.2 与 0.1.6-alpha.2 RPC/Event/Tool/Session-Control 形状与 mapper | 不能只 mock Domain           |
...
- 未发布版本（如 alpha.1）的 fixture 必须同时标注源码 tag/commit，并在能力矩阵中注明尚无真实运行包证据；已发布预发行版本（如 `0.1.2-alpha.2`、`0.1.2-alpha.3`、`0.1.2-alpha.4`、`0.1.2-alpha.5`、`0.1.3-alpha.2`、`0.1.5-alpha.1/.2/rc.1/rc.2`）也要记录 npm/tag/commit 与 live smoke 边界。
...
- `0.1.6-alpha.2` fixture 必须覆盖 `session/control` 的 `jobs`/`projections.inbox` 基线与增量、`next-turn`/`next-step` 到 Queue/Steer 的消息 ID 归约、alpha.1 `queues` 形状拒绝、`session/writer-held` 错误映射，以及取消/关闭时的 mux 资源释放；完整 managed DSH live smoke 已通过（14 个文件、20 个测试），但在 VS Code/Webview 现场回放和跨平台发布证据完成前不得标记为 `DONE`。
