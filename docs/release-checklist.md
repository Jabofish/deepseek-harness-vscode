...
当前发布版本是 `0.0.6`，运行时契约覆盖 DSH `0.1.0-rc.6`、`rc.7` 和 `rc.8`；任何非空未知版本标签进入兼容模式并显示警告。能力矩阵仍有 `PARTIAL`，以下未勾选项代表真实缺口，不应被“构建成功”替代。
...
- [ ] 真实 DSH rc.6/rc.7/rc.8 smoke matrix 通过，且未知版本警告降级通过。
