...
当前发布版本是 `0.0.8`，运行时契约覆盖已发布 DSH `0.1.0-rc.6` 至 `0.1.1-rc.2`，并包含 `0.1.2-alpha.1` 源码预适配和已发布 `0.1.2-alpha.2` 适配；alpha 不作为安装默认。任何非空未知版本标签进入兼容模式并显示警告。能力矩阵仍有 `PARTIAL`，以下未勾选项代表真实缺口，不应被“构建成功”替代。

## 功能

- [ ] `docs/capability-matrix.md` 所有核心能力为 DONE，并有代码/自动/live 证据。
- [x] 无用户可达路径抛出 `TodoImplementationError`。
- [ ] 未知 DSH 版本只显示兼容性警告并保留基本功能，已支持版本显示准确。
