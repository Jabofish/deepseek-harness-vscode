...
当前发布版本是 `0.0.8`，运行时契约覆盖已发布 DSH `0.1.0-rc.6` 至 `0.1.1-rc.2`，并包含未发布 `0.1.2-alpha.1` 的源码预适配；alpha 不作为安装默认。任何非空未知版本标签进入兼容模式并显示警告。能力矩阵仍有 `PARTIAL`，以下未勾选项代表真实缺口，不应被“构建成功”替代。
