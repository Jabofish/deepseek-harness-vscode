...
退出条件：契约测试可发现 rpc-map 漂移；连接未知版本会按最新可安全复用 wire 的 Adapter 优先进行只读兼容探测并进入警告降级，不能把 `alpha13`/`alpha132` 的 Session v2、`alpha151`/`alpha152`/`rc151`/`rc152` 的 Session v3、alpha132 的 subagent `delivery` 或 alpha152/rc151/rc152 的交付/目录事件当作未知版本回退；fixture 不含路径、Prompt 或 Secret。
