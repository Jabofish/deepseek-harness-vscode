...
这是功能范围的唯一清单。`DONE` 必须同时有代码、自动测试和所需的真实 DSH 运行证据；当前尚未满足发布退出条件的能力统一标为 `PARTIAL`，并在证据列写明缺口。已发布 rc.6–0.1.2-rc.1、`0.1.2-alpha.2`–`.5` 和 `0.1.3-alpha.2`，以及源码级 `0.1.2-alpha.1`、`0.1.3-alpha.1` 使用独立版本入口；未知运行时按最新可安全复用 wire 的 Adapter 优先进行只读兼容探测，`alpha13`/`alpha132` 的 Session v2 仅精确版本可用，所有运行时专属能力仍以 `CAPABILITY_UNAVAILABLE` 和兼容警告边界降级；没有提供的 RPC 不以空实现冒充完成。
...
托管启动回归修复（2026-08-29）：启动参数已迁移到版本化 `managedWebArguments` 契约；rc.6/rc.7 不传 `--no-open`，rc.8、rc.1、rc.2、`0.1.2-rc.1`、alpha.1、alpha.2、alpha.3、alpha.4、alpha.5、`0.1.3-alpha.1` 和 `.2` 仅在上游 Web Profile 声明后传入，未知版本不猜测可选 flag。实际 rc.6 隔离 smoke 已成功报告 loopback endpoint，并确认本次受管进程退出后端口关闭。
