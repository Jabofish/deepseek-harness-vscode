...
这是功能范围的唯一清单。`DONE` 必须同时有代码、自动测试和所需的真实 DSH 运行证据；当前尚未满足发布退出条件的能力统一标为 `PARTIAL`，并在证据列写明缺口。已发布 rc.6–0.1.1-rc.2、alpha.2、alpha.3、alpha.4 和 alpha.5，以及未发布 alpha.1 使用独立版本入口；未知运行时按最新已验证 Adapter 优先进行只读兼容探测，所有运行时专属能力仍以 `CAPABILITY_UNAVAILABLE` 和兼容警告边界降级；没有提供的 RPC 不以空实现冒充完成。
...
托管启动回归修复（2026-08-29）：启动参数已迁移到版本化 `managedWebArguments` 契约；rc.6/rc.7 不传 `--no-open`，rc.8、rc.1、rc.2、alpha.1、alpha.2、alpha.3、alpha.4 和 alpha.5 仅在上游 Web Profile 声明后传入，未知版本不猜测可选 flag。实际 rc.6 隔离 smoke 已成功报告 loopback endpoint，并确认本次受管进程退出后端口关闭。
...
| CN-05  | 断线恢复、事件续接、历史补洞、无重复事件                                                         | `dsh-adapter/stream-controller.ts`                                                                                                              | 断流/乱序/重复/补洞 fixture                        | PARTIAL | rc.6 mux/host WebSocket downlink、重连、序号去重和历史 hydrate 已实现；Loopback WebSocket 的合法/畸形/错误/取消/释放测试通过；本轮补齐 HTTP/网络/超时/取消错误的稳定映射与重试边界，未知 runtime 按最新已验证 Adapter 兼容探测并保留真实版本；仍缺断流/补洞 fixture、真实 Windows VS Code managed smoke 的完整事件流复核。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| CN-06  | alpha.1/alpha.2/alpha.3/alpha.4/alpha.5 Connection/Gateway 协议、Cookie 握手、多路复用与兼容投影 | `dsh-adapter/src/versions/alpha` + `versions/alpha2` + `versions/alpha3` + `versions/alpha4` + `versions/alpha5`                                | HTTP/WS envelope、错误、chunk、waterfall fixture   | PARTIAL | alpha.1 已按 `dsh-v0.1.2-alpha.1`/`cd5ef814` 适配 `/api/<namespace>/<method>`、`/api/remote.mux`、`$events`、Session/Workspace follow、packed chunk rows、Goal/Model/Preset 映射和 Host-only Cookie；alpha.2 按已发布 `dsh-v0.1.2-alpha.2`/`0a53fb55` 独立归一化 namespaced error、`ignorable` 事件和可选 agent-preset 插件组合；alpha.3 按已发布 `dsh-v0.1.2-alpha.3`/`dd6322d6` 保持同一 wire mapper，补齐精确版本入口、`--no-open` 与 alpha PTC 模式；alpha.4 按 tag `dsh-v0.1.2-alpha.4`/`4e84901e` 核对 Session branded sequence/offset、inherited-event 到 `seedLength` 的 wire 投影及 Subagent 内部投递重构，保持 Connection/Gateway/Remote wire；alpha.5 按 tag `dsh-v0.1.2-alpha.5`/`db6bdc35` 及当前 `master`/`49a606bc` 增加独立精确入口并沿 alpha.4 线复用已验证 transport/mapper，alpha.4 不再作为未知回退；alpha.1/alpha2/alpha3/alpha4/alpha5 契约测试覆盖成功、业务错误、畸形响应、错误 fail-closed、混合 prompt、取消和资源释放；alpha.2 已有基础包 smoke，alpha.3 已验证 Web Profile endpoint、Cookie 登录和 `session/list` envelope，alpha.4/alpha.5 尚未完成真实 Web Profile/remote.mux/Session/Workspace follow，仍缺断线恢复和 VS Code Webview 回放；alpha.1 无运行包，因此保持 PARTIAL。                                                                                                                                                                                                       |
...
- 版本层：`versions/rc6`、`versions/rc7`、`versions/rc8` 与受控兼容探测；运行时定位允许任何非空未知版本标签，Probe 按显式优先级选择最新已验证 Adapter，保留真实版本、Adapter 身份并把警告安全传给 Webview。
...
- 版本层：新增 `packages/dsh-adapter/src/versions/rc11`，依赖固定到 `@deepseek-ai/dsh-host-apiproxy@0.1.1-rc.1`。rc.6/rc.7/rc.8 继续走原有 Adapter；未知非空版本按最新已验证 Adapter 优先进行兼容探测，不发送 rc.1 专有字段。
...
- 版本层：新增 `packages/dsh-adapter/src/versions/rc12`，依赖固定到 `@deepseek-ai/dsh-host-apiproxy@0.1.1-rc.2`。rc.1 继续由 rc11 Adapter 发送完整空白会话复用字段；rc.2 在匹配到官方条件时由 Webview 直接 `session.open`，否则只使用不含 `reuseWorkspaceBlank` 的创建请求；rc.6–rc.8 与未知版本的最新 Adapter 优先兼容探测路径保持独立。
...
- CN-06：本地上游仓库 `D:\CS\dsh` 已同步到当前 `master` 提交
...
- 选择规则：运行时版本为空时沿用未版本化候选的既有探测；运行时版本为非空且不在
  ...
  `0.1.2-alpha.6` 会先尝试 alpha.5 Adapter，只有其只读 `session/list` 契约探测拒绝后才继续更旧候选；
  已知 alpha.4 不进入此分支。
...
- 自动证据：`packages/dsh-adapter/test/probe.spec.ts` 覆盖最新优先、显式优先级、精确-only 候选跳过、
  ...
  保留真实版本、alpha.5 Adapter 身份和兼容警告；`adapter-chain.spec.ts` 锁定两条 family 线性继承链，
