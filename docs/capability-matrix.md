...
| SS-01 | Session 新建、列表、分页、历史、搜索、重命名、分叉、归档、删除、恢复 | session repository、use cases、SessionDrawer              | 完整生命周期、分页、搜索边界与降级、改名回执、归档列表/恢复/确认删除、归档/删除与新建/分叉期间改选导航不被旧请求覆盖（含分叉重命名失败）和真实 DSH 回放                                                                                                                                             | PARTIAL |
...
| MD-01 | 动态 Provider/Model/Reasoning 发现、选择、不可用状态和会话应用       | model repository、model features                          | 动态目录、未知项、逐 Provider 失败原因（宿主原文且不阻断可用分组）、目录失效刷新、读取失败原因与重读入口、`routable` 判定下的惰性输入、推理强度名/声明默认与目录缺席时的 `provider/model` 标识、由持久投影组合出的会话自选路由与据此的 `routable`、未选时按路由陈述展示且不写回配置、真实 DSH 回放  | PARTIAL |
| AG-01 | 标准/code/minimal/cordis、用户 Preset、Tools native/code/both        | session config、picker、settings                          | alpha162 Cordis 待激活请求可显式拒绝，页面查询提示需 DSH Web/停止回合（含 Remote → 流控制器 → 权限请求/拒绝回归测试，未实现浏览器执行）；动态能力、持久化、未知值、插件清单展示宿主 `agentPresets` 的组成/条件启用/默认标记/损坏原因和真实 DSH 回放                                                 | PARTIAL |
| PM-01 | `read-only`/`workspace-write`/`full-access`/custom 权限与审批一致    | session config、ApprovalCard                              | 0.1.6 alpha 独立权限目录与失效刷新、目录失败降级且保留当前权限（取消继续上抛）、Auto 风险确认（契约/回归测试及真实 alpha.2 目录/切换通过，VS Code 现场待验证）、审批一次性、过期、取消、审批命令预览（`callId` 配对调用卡）和真实 DSH 回放                                                          | PARTIAL |
...
| ST-01 | DSH Settings Schema、读取、更新、替换以及 live/restart 语义          | settings repository、SettingsDrawer                       | schema 负面路径、持久化、按宿主完整字段路径映射的覆盖状态与逐字段恢复默认（契约测试覆盖 false、空字符串、数组与未覆盖相邻字段）和真实 DSH 回放                                                                                                                                                      | PARTIAL |
...
| PG-01 | Plugin Inventory、能力、显式配置、重连/重启提示和未知插件降级        | plugin repository、PluginInventory                        | 动态目录、未知插件和真实 DSH 回放；插件目录在 plugin-manager/changed 与重连后刷新，丢弃过期读取（自动测试覆盖，真实 UI 待验）；配置卡片由宿主 settings schema 派生（命名空间、字段与凭据引用都不是内置清单），按字段路径标记覆盖状态并可逐字段恢复默认，非凭据对象/数组可 JSON 编辑、拒绝非有限数字 | PARTIAL |
| TL-01 | 通用 Tool Card 与 Shell/Edit/Search/LSP/MCP 等结构化 Renderer        | UI registry、rc6 tool mapping                             | 官方 catalog、未知工具、嵌套调用、无 `view` 的 alpha 线按 `meta` 形状与调用参数派生卡片、已结算 shell 行在时间线结算出输出与退出码、工具结果持久图片引用（含 PTC 子调用）的校验展示与展开重试、真实 Webview 回放                                                                                    | PARTIAL |
...
- Plugin Manager 安装/卸载/启停、Cordis 全局管理、Office 转换预览和默认关闭的 Schedule 不作为聊天插件必需功能，继续交由 DSH 或宿主工具处理。
- alpha162 已读取上游 `workspace/changes` 的进程内权威 summary/diff，审查按现有工作区所有权限制展示；打开审查时最多回读 100 页历史，进程重启后的快照不可恢复。空快照或全部路径被过滤时通过会话级失效事件刷新 Changes 列表（Host/Webview 回归测试）。权威刷新失败时保留本地列表并显示未更新提示，取消/权限失败不降级。读取失败统一为 AppError，覆盖网络、读取、超时、取消、关闭和畸形 JSON；404 仍表示快照不存在。未复制上游独立审查 tab 或工作区外文件面。
