...
这是功能范围的唯一清单。`DONE` 必须同时有代码、自动测试和所需的真实 DSH 运行证据；当前尚未满足发布退出条件的能力统一标为 `PARTIAL`，并在证据列写明缺口。已发布 `0.0.1-rc.1/.2/.5`、`0.1.0-rc.2/.3`、rc.6–0.1.2-rc.1、`0.1.2-alpha.2`–`.5`、`0.1.3-alpha.2` 和 `0.1.5-alpha.1/.2/rc.1`，以及源码级 `0.1.2-alpha.1`、`0.1.3-alpha.1` 使用独立版本入口；未知运行时按最新可安全复用 wire 的 Adapter 优先进行只读兼容探测，`alpha13`/`alpha132` 的 Session v2 和 `alpha151`/`alpha152`/`rc151` 的 Session v3 仅精确版本可用，所有运行时专属能力仍以 `CAPABILITY_UNAVAILABLE` 和兼容警告边界降级；没有提供的 RPC 不以空实现冒充完成。安装器当前默认精确支持集合中的最新 `0.1.5-rc.1`。
