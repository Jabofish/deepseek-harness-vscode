...
这是功能范围的唯一清单。`DONE` 必须同时有代码、自动测试和所需的真实 DSH 运行证据；当前尚未满足发布退出条件的能力统一标为 `PARTIAL`，并在证据列写明缺口。已发布 rc.6–0.1.2-rc.1、alpha.2、alpha.3、alpha.4 和 alpha.5，以及未发布的 alpha.1 与最新 `0.1.3-alpha.1` 源码快照使用独立版本入口；未知运行时按最新可安全复用 wire 的 Adapter 优先进行只读兼容探测，`alpha13` 的 Session v2 仅精确版本可用，所有运行时专属能力仍以 `CAPABILITY_UNAVAILABLE` 和兼容警告边界降级；没有提供的 RPC 不以空实现冒充完成。
...
托管启动回归修复（2026-08-29）：启动参数已迁移到版本化 `managedWebArguments` 契约；rc.6/rc.7 不传 `--no-open`，rc.8、rc.1、rc.2、`0.1.2-rc.1`、alpha.1、alpha.2、alpha.3、alpha.4、alpha.5 和 `0.1.3-alpha.1` 仅在上游 Web Profile 声明后传入，未知版本不猜测可选 flag。实际 rc.6 隔离 smoke 已成功报告 loopback endpoint，并确认本次受管进程退出后端口关闭。
...
| CN-06  | `0.1.2-rc.1`/alpha.1/alpha.2/alpha.3/alpha.4/alpha.5/`0.1.3-alpha.1` Connection/Gateway 协议、Cookie 握手、多路复用与兼容投影 | `dsh-adapter/src/versions/rc13` + `versions/alpha` + `versions/alpha2` + `versions/alpha3` + `versions/alpha4` + `versions/alpha5` + `versions/alpha13` | HTTP/WS envelope、错误、Session v0/v2、assistant stream、waterfall fixture | PARTIAL | 已发布 `0.1.2-rc.1` 按 npm/tag `a66e4702` 建立 `versions/rc13` 精确入口，保留 alpha.5 的 Cookie、remote.mux、packed history 和 `seedLength` Session v0；alpha.1 已按 `dsh-v0.1.2-alpha.1`/`cd5ef814` 适配 `/api/<namespace>/<method>`、`/api/remote.mux`、`$events`、Session/Workspace follow、packed chunk rows、Goal/Model/Preset 映射和 Host-only Cookie；alpha.2 按已发布 `dsh-v0.1.2-alpha.2`/`0a53fb55` 独立归一化 namespaced error、`ignorable` 事件和可选 agent-preset 插件组合；alpha.3 按已发布 `dsh-v0.1.2-alpha.3`/`dd6322d6` 保持同一 wire mapper，补齐精确版本入口、`--no-open` 与 alpha PTC 模式；alpha.4 按 tag `dsh-v0.1.2-alpha.4`/`4e84901e` 核对 Session branded sequence/offset、inherited-event 到 `seedLength` 的 wire 投影及 Subagent 内部投递重构，保持 Connection/Gateway/Remote wire；alpha.5 按 tag `dsh-v0.1.2-alpha.5`/`db6bdc35` 及当前 `master`/`49a606bc` 增加独立精确入口并沿 alpha.4 线复用已验证 transport/mapper，alpha.4 不再作为未知回退；最新上游 tag `dsh-v0.1.3-alpha.1`/`d347e703908d0406b7a7ef80e3a0e594d86b2215` 的 Session v2 已建立 `versions/alpha13` 独立版本缝，严格校验 `isSeeded` header、event-only history、`assistantStream: true` 的 `start/chunk/end` 修订序列、压缩 assistant baseline 与 durable settlement，并在 Timeline 以 attempt/index 去重且不伪造 durable seq；`rc13-contract.spec.ts`、`alpha13-contract.spec.ts`、`adapter-chain.spec.ts`、`launch-contract.spec.ts`、`reducer.spec.ts` 及既有 alpha 回归已通过；该版本当前未作为 npm 安装默认，真实 `0.1.2-rc.1` Session 回放、`0.1.3-alpha.1` Web Profile/remote.mux/长回答断线恢复/VS Code Webview 回放尚未执行，因此继续保持 PARTIAL。 |
...
## 2026-09-06 DSH `0.1.3-alpha.1` 上游同步与 Session v2 适配证据

- CN-06：上游仓库 `D:\CS\dsh` 的远端 `origin/master` 已核对为 tag
  `dsh-v0.1.3-alpha.1`、提交 `d347e703908d0406b7a7ef80e3a0e594d86b2215`，源码 manifest
  版本为 `0.1.3-alpha.1`。npm 当前没有同版本可安装包，因此本仓库保留已发布 npm 依赖不变，
  以独立 `versions/alpha13` 适配源码契约；默认安装版本仍不切换到未发布快照。
...
- 选择规则：运行时版本为空时沿用未版本化候选的既有探测；运行时版本为非空且不在
  ...
  `0.1.2-alpha.6` 会跳过不能安全协商 Session v2 的 alpha13，先尝试可复用 v0 wire 的 alpha.5
...
- 自动证据：`packages/dsh-adapter/test/probe.spec.ts` 覆盖最新优先、显式优先级、精确-only 候选跳过、
  ...
  保留真实版本、alpha.5 Adapter 身份和兼容警告，`alpha13-contract.spec.ts` 锁定 alpha13 精确-only
  和未知版本回退到 alpha.5；`adapter-chain.spec.ts` 锁定两条 family 线性继承链，
