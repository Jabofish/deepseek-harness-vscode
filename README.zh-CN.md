...
| DeepSeek Harness | 已发布 Host/Web API `0.1.0-rc.6` 至 `0.1.1-rc.2`，并提供源码级 `0.1.2-alpha.1` 与已发布 `0.1.2-alpha.2` 的兼容适配层（alpha 不作为安装默认） |
| 未知版本         | 任何能报告非空版本标签的版本都会尝试以兼容模式连接，并在界面显示警告                                                                         |
