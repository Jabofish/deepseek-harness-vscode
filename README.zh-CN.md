...
| DeepSeek Harness | 已发布 CLI/Web API `0.1.0-rc.6` 至 `0.1.2-rc.1`，已发布 `0.1.2-alpha.2`/`0.1.2-alpha.3`/`0.1.2-alpha.4`/`0.1.2-alpha.5` 和 `0.1.3-alpha.2`，并保留 `0.1.2-alpha.1`、`0.1.3-alpha.1` 的源码级适配（预发布版本不作为安装默认） |
| 未知版本         | 任何非空版本标签都会优先由可安全复用已验证 wire 的 Adapter 探测；alpha13/alpha132 Session v2 仅精确匹配，未知运行时回退到已验证的 alpha5 v0 Adapter，并保留真实版本和兼容警告                                                |
