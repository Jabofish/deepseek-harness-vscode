...
| DeepSeek Harness | 已发布 Host/Web API `0.1.0-rc.6` 至 `0.1.1-rc.2`，并提供源码级 `0.1.2-alpha.1` 与已发布 `0.1.2-alpha.2`/`0.1.2-alpha.3`/`0.1.2-alpha.4`/`0.1.2-alpha.5` 的兼容适配层（alpha 不作为安装默认） |
| 未知版本         | 任何非空版本标签都会优先用最新已验证 Adapter 探测；成功后保留真实版本，以兼容模式连接并在界面显示警告                                                                                        |
