...
当前适配已发布的 DeepSeek Harness `0.1.0-rc.6` 至 `0.1.1-rc.2` Host/Web API，并已针对尚未封包的上游 `0.1.2-alpha.1` 源码预置独立 Connection/Gateway 适配层；alpha 仍不作为扩展安装默认版本。任何能报告非空版本标签的未知版本也会尝试以兼容模式连接，并在界面显示警告。支持 Windows、Linux 和 macOS；从扩展内安装 DSH 时需要 Node.js `22.19+`。模型、工具和高级 Agent 能力以当前 DSH 实例为准，未提供的能力会明确提示。

Currently targets the published DeepSeek Harness `0.1.0-rc.6` through `0.1.1-rc.2` Host/Web API on Windows, Linux, and macOS, with a separate prepared Connection/Gateway adapter for the unpublished upstream `0.1.2-alpha.1` source. The alpha is not the extension's install default. Any unknown version that reports a non-empty label is also attempted in compatibility mode and surfaced with a warning. Installing DSH from the extension requires Node.js `22.19+`. Available models, tools, and advanced Agent capabilities depend on the connected DSH instance.
