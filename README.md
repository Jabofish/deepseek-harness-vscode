...
| DeepSeek Harness | Published CLI/Web API `0.1.0-rc.6` through `0.1.2-rc.1`, published `0.1.2-alpha.2`/`0.1.2-alpha.3`/`0.1.2-alpha.4`/`0.1.2-alpha.5`, released `0.1.3-alpha.2`, and latest upstream tag/alpha npm dist-tag `0.1.5-alpha.1`; source-level adapters are retained for every known tag (pre-release versions are not the install default) |
