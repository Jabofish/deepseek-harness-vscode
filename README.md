...
| DeepSeek Harness | Published CLI/Web API `0.1.0-rc.6` through `0.1.2-rc.1`, published `0.1.2-alpha.2`/`0.1.2-alpha.3`/`0.1.2-alpha.4`/`0.1.2-alpha.5`, and released `0.1.3-alpha.2`; source-level adapters are retained for `0.1.2-alpha.1` and `0.1.3-alpha.1` (pre-release versions are not the install default) |
| Unknown versions | Any non-empty label is probed with the newest adapter that can safely reuse a verified wire; the alpha13/alpha132 Session v2 adapters are exact-only, so unknown runtimes fall through to the verified alpha5 v0 adapter and surface a warning                                                  |
