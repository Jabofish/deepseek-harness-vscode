...
| DeepSeek Harness | Published CLI/Web API `0.0.1-rc.1`/`.2`/`.5`, `0.1.0-rc.2`/`.3`, `0.1.0-rc.6` through `0.1.2-rc.1`, published `0.1.2-alpha.2`/`0.1.2-alpha.3`/`0.1.2-alpha.4`/`0.1.2-alpha.5`, released `0.1.3-alpha.2`, `0.1.5-alpha.1`/`.2`/`rc.1`/`rc.2`, alpha-channel `0.1.6-alpha.1`/`.2` and `0.1.7-alpha.1`; source-level adapters are retained for every known tag and the installer installs the exact version `0.1.5-rc.2`, never a dist-tag (npm's `latest` and `next` currently point there, `alpha` at `0.1.7-alpha.1`) |
