...
export const SUPPORTED_DSH_VERSIONS = [
  ...
  '0.1.6-alpha.2',
  // 0.1.7-alpha.1 moves Session history to V4, removes Jobs from Session
  // Control, and exposes the standalone Job Controller stream.
  '0.1.7-alpha.1',
  '0.1.7-alpha.2',
  '0.1.7-rc.1',
  '0.1.7-rc.2',
...
const DSH_PACKAGE_VERSION_PATTERN =
...
export function isDshPackageVersion(value: string): boolean {
  return DSH_PACKAGE_VERSION_PATTERN.test(value)
}

/** A SemVer-looking fragment inside a non-SemVer label is not exact identity evidence. */
export function isMalformedDshVersionHint(value: string | undefined): boolean {
  if (value === undefined || value.trim() === '') return false
  return /\b\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?\b/u.test(value) && !isDshPackageVersion(value)
}
...
export function normalizeDshVersion(value: string | undefined): string | undefined {
  ...
  if (isDshPackageVersion(value)) return value
...
export interface DshTransport {
  /** Exact alpha/rc session-controller profile supports turn-aligned history pages. */
