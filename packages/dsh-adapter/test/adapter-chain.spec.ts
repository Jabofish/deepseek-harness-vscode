...
describe('version adapter family chains', () => {
  it('keeps the legacy rc chain linear and newest-first priorities monotonic', () => {
    ...
    expect([rc6, rc7, rc8, rc11, rc12].map((adapter) => adapter.compatibilityPriority)).toEqual([
      30, 40, 50, 60, 70,
    ])
    expect([rc6, rc7, rc8, rc11, rc12].map((adapter) => adapter.protocolVersion)).toEqual([
      'rc6',
      'rc7',
      'rc8',
      'rc11',
  ...
  it('keeps the alpha chain linear without crossing the rc transport boundary', () => {
    ...
    expect([alpha1, alpha2, alpha3, alpha4, alpha5].map((adapter) => adapter.supportedVersion)).toEqual([
      '0.1.2-alpha.1',
      '0.1.2-alpha.2',
      '0.1.2-alpha.3',
      '0.1.2-alpha.4',
    ...
    expect([alpha1, alpha2, alpha3, alpha4, alpha5].map((adapter) => adapter.compatibilityPriority)).toEqual([
      80, 90, 100, 110, 120,
    ])
    expect([alpha1, alpha2, alpha3, alpha4, alpha5].map((adapter) => adapter.protocolVersion)).toEqual([
      'alpha1',
      'alpha2',
      'alpha3',
      'alpha4',
